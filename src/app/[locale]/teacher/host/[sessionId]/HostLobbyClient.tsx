...
export default function HostLobbyClient({ sessionId, initialSession, initialParticipants }: any) {
  ...
  if (!session) return <div className="p-8 font-bold text-center">Loading session...</div>;

  const handleSettingsChange = async (newFeedbackLevel: string, newRandom: boolean) => {
      if (newFeedbackLevel === "SHOW_NOTHING") newRandom = true;
      setSession({...session, feedbackLevel: newFeedbackLevel, randomNicknames: newRandom});
      await updateSessionSettingsAction(sessionId, newFeedbackLevel, newRandom);
  }
  ...
  if (session.status === "WAITING") {
            ...
            <div className="space-y-6">
              <div>
                ...
                <select 
                   ...
                   onChange={(e) => handleSettingsChange(e.target.value, session.randomNicknames)}
              ...
              <div className="flex items-center justify-between bg-gray-50 p-6 rounded-xl border-2 border-gray-200 hover:border-brand-purple/50 transition">
                ...
                <input 
                  ...
                  onChange={(e) => handleSettingsChange(session.feedbackLevel, e.target.checked)}
    ...
    return (
      <div className="max-w-4xl mx-auto w-full flex-1 flex flex-col items-center justify-center space-y-4 text-center py-8">
        ...
        <p className="text-2xl text-gray-500 font-medium tracking-wide mt-4">{t("enterCodeAt")} <strong>localhost:3000/join</strong> :</p>
        <div className="bg-white border-4 border-brand-purple rounded-3xl p-10 shadow-2xl relative overflow-hidden group my-8">
          <div className="text-7xl sm:text-8xl md:text-[10rem] font-black text-brand-purple tracking-widest leading-none drop-shadow-md">
            {session.code}
          </div>
        </div>
        ...
        <div className="w-full bg-white rounded-3xl p-8 mt-8 shadow-sm border-2 border-gray-100">
          ...
          <div className="flex flex-wrap gap-4 justify-center min-h-[6rem] items-center">
            ...
            {participants.map((p: any) => (
              <div 
                key={p.id} 
                className="bg-brand-purple text-white font-bold px-6 py-3 rounded-full text-2xl shadow-md whitespace-nowrap overflow-hidden text-ellipsis border-2 border-purple-700 transform hover:scale-105 transition animate-bounce-in"
              >
                  ...
                  <div key={p.id} className="flex flex-col bg-gray-50 p-5 rounded-2xl border-2 border-gray-200 shadow-sm transition-all hover:scale-[1.01] overflow-hidden relative">
                    ...
                    <div className="relative flex justify-between items-center z-10 w-full">
                      <div className="flex flex-col">
                        <span className="font-bold text-2xl text-gray-700">{i+1}. {session.randomNicknames ? p.randomName : p.nickname}</span>
        ...
        {winner && (
         ...
         <div className="w-full bg-white rounded-3xl p-8 mt-8 shadow-sm text-left border-4 border-gray-100">
          ...
          <div className="space-y-3">
             {sorted.map((p, i) => (
                <div key={p.id} className="flex justify-between items-center bg-gray-50 p-4 rounded-xl border-2 border-gray-100">
                  <span className="font-bold text-xl">{i+1}. {session.randomNicknames ? p.randomName : p.nickname}</span>
